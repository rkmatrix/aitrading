...
class LiveReloadMixin:
    ...
    def reload_policies(cls, registry: dict) -> None:
        ...
        logger.info("SmartOrderRouter: reloaded registry with %d entries", len(cls._registry))
    ...
    def _load_json(path: str | Path) -> dict | None:
        try:
            ...
            logger.error("Router LiveReload read failed for %s: %s", path, ex)
            return None

    def get_routing_policy(self, name: str) -> dict | None:
